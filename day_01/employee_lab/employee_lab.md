...
#### Create a subclass of `Employee` called `Staff` in a package called management.
- Create a class for Staff.
...
#### Create a subclass of `Staff` called `Director` in the management package.
- Add a private property to store a double value `budget`.
- Create a constructor for `Director` that includes the parameters needed for `Staff` and the `budget` parameter.
